...
int ValueJudge(int Map[21][26], int snake_y, int snake_x)
{
	...
	{
		diretion[0] = Distance(food_x, food_y, snake_x + 1, snake_y);
	}
	if ((Map[snake_y][snake_x - 1] == 1) || (Map[snake_y][snake_x - 1] > 100))
	{
		diretion[2] = 999999;
	...
	{
		diretion[2] = Distance(food_x, food_y, snake_x - 1, snake_y);
	}
	...
	{
		diretion[1] = Distance(food_x, food_y, snake_x, snake_y - 1);
	}
	if ((Map[snake_y + 1][snake_x] == 1) || (Map[snake_y + 1][snake_x] > 100))
	{
		diretion[3] = 999999;
	...
	{
		diretion[3] = Distance(food_x, food_y, snake_x, snake_y + 1);
	}
	...
	if(level==3)
	{
		...
		BL:
			...
			for (;y < 21;y++)
			{
				for (;x < 26;x++)
				{
					if (Map[y][x] == 0 && (x > 14 || x < 12) && (y > 11 || y < 9)&&x>2&&x<24&&y>2&&y<19)
					{
						Mapp = &Map[y][x];
						*Mapp = 1;
						i++;
...
void Draw(int array[21][26], int score, int level)
{
	for (int y = 1;y < 21;y++)
	{
		for (int x = 1;x < 26;x++)
		{
			...
			case 6:setfillcolor(LIGHTCYAN);break;
			}
			if (array[y][x] > 100)
			{
				setfillcolor(GREEN);
...
int ControlSnake(int score, int Map[21][26], int direction, int* length, int* head_x, int* head_y, bool* crash, bool* next, int level, int rank[], int loadlevel, int* p_wisdom)
{
	*crash = false;
	switch (direction)
	{
...
void main()
{
	...
	int Map[21][26];//创建初始地图,值为0代表空地，1为墙壁，2为食物，3为传送门，4为炸弹，5为毒草，6为智慧草 大于100为蛇身；0白 1黄 2红 3蓝 4灰 5紫 >100绿
	...
	while (level <= 3)
	{
		...
		int wisdom = 0;/*智慧草 */ int* p_wisdom = &wisdom;
		while (key != 'p')
		{
			if ((timeline % (6 - diffcult)) == 0)
			{
				...
				if (ControlSnake(score, Map, snake.direction, p_length, p_head_x, p_head_y, p_crash, p_next, level, rank, loadlevel, p_wisdom) == 1)
				{
					timeplay = 1;
					play_x = snake.head_x;
					play_y = snake.head_y;
				...
				EndBatchDraw();
				//积分系统
				score += (snake.length*5) * diffcult / 1.5;
			}
			timeline++;
			if (timeline == 481)
			{
